...
const Article = ({ article }) => {
    ...
    useEffect(() => {
        ...
        const allArticles = document.querySelectorAll(`.${styles.article}`);
    
        if (articleElement && imageElement && contentElement) {
            // Révélation de l'image
            gsap.fromTo(
                imageElement,
                { clipPath: "inset(100% 0 0 0)", opacity: 0.5, scale: 1.1, filter: "blur(10px)" },
                {
    ...
    return (
        <li ref={articleRef} key={article.id} className={styles.article}>
            <div ref={imageRef} className={`${styles.article__image} ${styles.image_background} mobile`}>
                <img
            ...
            <div ref={imageRef} className={`${styles.article__image} ${styles.image_background} desktop`}>
                <img
                    className={styles.img}
                    src={imageDesktopUrl}
